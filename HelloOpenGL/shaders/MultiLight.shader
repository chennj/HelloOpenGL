...
vec3 CalcLightDirectional(LightDirectional light, vec3 normal, vec3 dirToCamera)
{
	...
	vec3 diffuseColor = diffuseIntensity * light.color * texture(material.diffuse, v_TexCoord).rgb;
	...
	vec3 specularColor = specularIntensity * light.color * texture(material.specular, v_TexCoord).rgb;
